...
import { groups, getToolsInGroup } from "../../../docs/arcgis/_data/arcgisDocs";
 ...
 * behaviour comes for free and no open/closed state has to be tracked. With 11
 * groups and up to 24 tools in one of them, only the active group starts open —
 * expanding all of them would put 118 links in a sticky column.
 */
const DocsSidebar = ({ mode = "anchors", activeSlug = null, activeGroupId = null }) => {
  ...
  return (
    <nav
      ...
      className="hidden lg:block lg:sticky lg:top-28 lg:max-h-[calc(100vh-8rem)] lg:self-start lg:overflow-y-auto"
    >
      <Link
        href="/docs/arcgis"
          ...
          return (
            <li key={group.id}>
              {isAnchors ? (
                <a
                  href={`#group-${group.id}`}
                  className="flex items-center justify-between gap-2 rounded-lg px-2 py-1.5 text-sm text-brand-text/70 transition-colors hover:bg-brand-blue/5 hover:text-brand-orange focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-orange/60 dark:text-gray-300 dark:hover:bg-white/5"
                >
                  <span lang="en">{group.name}</span>
                  <span className="text-xs text-brand-text/45 dark:text-gray-500">
                    {toolsInGroup.length}
                  </span>
                </a>
              ) : (
                <details open={isActiveGroup} className="group/details">
                  <summary className="flex cursor-pointer list-none items-center justify-between gap-2 rounded-lg px-2 py-1.5 text-sm font-display font-600 text-brand-text/80 transition-colors hover:bg-brand-blue/5 hover:text-brand-orange focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-orange/60 dark:text-gray-200 dark:hover:bg-white/5">
                    <span lang="en">{group.name}</span>
                    <span className="text-xs text-brand-text/45 dark:text-gray-500">
                      {toolsInGroup.length}
                    </span>
