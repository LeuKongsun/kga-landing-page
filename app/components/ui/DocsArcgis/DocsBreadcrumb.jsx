...
/**
 * The trail runs Programs > KGA Toolbox > Documentation, matching the path the
 * add-in's product page uses to get here — /kga-toolbox links to /docs/arcgis
 * in two places, so the product page is a real step in the journey.
 */
  ...
  return (
    <nav aria-label="Breadcrumb" className="mb-6">
      <ol className="flex flex-wrap items-center gap-1 text-sm text-brand-text/60 dark:text-gray-400">
        ...
        <li>
          <Link href="/kga-toolbox" className={crumbLink}>
            KGA Toolbox
          </Link>
        </li>
        <ChevronRight className="h-3.5 w-3.5 flex-shrink-0" aria-hidden="true" />
        {group || tool ? (
          <li>
            <Link href="/docs/arcgis" className={crumbLink}>
        ...
        {group && (
          <>
            <ChevronRight className="h-3.5 w-3.5 flex-shrink-0" aria-hidden="true" />
            <li>
              <Link href={`/docs/arcgis#group-${group.id}`} className={crumbLink}>
